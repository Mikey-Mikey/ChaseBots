...
local function DrawPlayerRow(ply, x, y, w, h)
    ...
    if not ply:Alive() then
        ...
        draw.SimpleText("Dead", "ScoreboardInfo", x + w * 0.75 - 48, y + h / 2 + 2, Color(0,0,0,127), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
    ...
    draw.SimpleText(name, "ScoreboardInfo", x + 10, y + h / 2 + 2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

    local role = table.KeyFromValue(ROLE_COLORS, roleColor)
    draw.SimpleText(role, "ScoreboardInfo", x + w * 0.5, y + h / 2 + 2, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local ping = ply:Ping()
    draw.SimpleText(ping, "ScoreboardInfo", x + w - 48, y + h / 2 + 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
